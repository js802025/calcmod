...
public class Random {
    ...
    public static LiteralArgumentBuilder<FabricClientCommandSource> register(LiteralArgumentBuilder<FabricClientCommandSource> command) {
        ...
        .then(ClientCommandManager.argument("max", StringArgumentType.greedyString()).executes(ctx -> {
            String[] message = execute(StringArgumentType.getString(ctx, "max"));
        ...
        .then(ClientCommandManager.literal("minmax").then(ClientCommandManager.argument("min", StringArgumentType.string()).then(ClientCommandManager.argument("max", StringArgumentType.greedyString()).executes(ctx -> {
            String[] message = execute(StringArgumentType.getString(ctx, "min"), StringArgumentType.getString(ctx, "max"));
            CalcCommand.sendMessage(ctx.getSource(), message);
            return 1;
        }))))
    ...
    public static String[] execute(String... range) {
        if (range.length == 1) {
        double maxInt = CalcCommand.getParsedExpression(range[0]);
        String random = nf.format(Math.round(Math.random() * maxInt));
        return new String[] { "Random number between 0 and " + range[0] + " is ", random };
        } else if (range.length == 2 ) {
            double max = CalcCommand.getParsedExpression(range[1]);
            double min = CalcCommand.getParsedExpression(range[0]);
            String random = nf.format(Math.round((Math.random() * ((max - min) + 1)) + min));

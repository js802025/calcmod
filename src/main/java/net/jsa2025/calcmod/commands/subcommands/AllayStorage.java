...
public class AllayStorage {
    ...
    public static LiteralArgumentBuilder<FabricClientCommandSource> register(LiteralArgumentBuilder<FabricClientCommandSource> command) {
        ...
        .then(ClientCommandManager.literal("allaystorage").then(ClientCommandManager.argument("itemsperhour", StringArgumentType.greedyString()).executes((ctx) -> {
            String[] message = execute((ServerCommandSource) ctx.getSource(), StringArgumentType.getString(ctx, "itemsperhour"));
    ...
    public static LiteralArgumentBuilder<ServerCommandSource> registerServer(LiteralArgumentBuilder<ServerCommandSource> command) {
        ...
        .then(CommandManager.literal("allaystorage").then(CommandManager.argument("itemsperhour", StringArgumentType.greedyString()).executes((ctx) -> {
            String[] message = execute(ctx.getSource(), StringArgumentType.getString(ctx, "itemsperhour"));
    ...
    public static String[] execute(ServerCommandSource commandSource, String itemsperhour) {
        double rates = CalcCommand.getParsedExpression(commandSource.getPlayer().getBlockPos(), itemsperhour, 1);

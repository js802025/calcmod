...
public class SignalToItems {
    ...
    public static LiteralArgumentBuilder<CommandSourceStack> register(LiteralArgumentBuilder<CommandSourceStack> command) {
        ...
        .then(Commands.argument("signal", StringArgumentType.greedyString()).executes((ctx) -> {
            CalcMessageBuilder message = execute(ctx.getSource().getExecutor(), StringArgumentType.getString(ctx, "container"), StringArgumentType.getString(ctx, "signal"));
...
//    }
    ...
    public static CalcMessageBuilder execute(Entity player, String container, String signal) {
        ...
        if (strength > 15) {
            message.addString("\n<red>Error: Signal Strength out of range (0, 15)");
        }
        return message;
    ...
    public static String helpMessage = """
        <aqua><bold>Signal To Items:<reset><white>
           Given a container and a desired comparator signal strength <gray><italic>(can be in expression form)<reset><white>, returns the number of items needed to achieve that signal strength.
            <yellow>Usage: /calc signaltoitems <container> <signal><white>
                """;
